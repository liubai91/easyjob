 ...
 * @see #setConcurrent
 */
public class MethodInvokingJobDetailLogSupportFactoryBean extends ArgumentConvertingMethodInvoker
	...
	public static class MethodInvokingJob implements Job {
		...
		protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
			try {
				ThreadLocalAccessUtils.setJobContextForCurrentThread(context);
				context.setResult(this.methodInvoker.invoke());
			} catch (Throwable e) {
				ThreadLocalAccessUtils.setJobException(e);
				JobDataMap jobdata = context.getMergedJobDataMap();
				JobExceptionStrategy strategy = (JobExceptionStrategy) jobdata.get("strategy");
				if(strategy==JobExceptionStrategy.BREAK){
					JobExecutionException e2 = new JobExecutionException("任务执行过程中抛出未捕获异常");
